...
class Donation extends React.Component {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        if(prevState.localUpdate){
            ...
            return { 
                ...
                balance:parseFloat(nextProps.pageData.loggedInUserDetails ? nextProps.pageData.loggedInUserDetails.wallet : 0)
            }
        }else if (parseFloat(nextProps.pageData.loggedInUserDetails ? nextProps.pageData.loggedInUserDetails.wallet : 0) != prevState.balance) {
            return { 
                id:null,
                gateways:null,
                custom_url:nextProps.custom_url,
                ...
                axios.post(url, formData)
                    .then(response => {
                        if (response.data.error) {
                            this.setState({id:null,localUpdate:false})
                            ...
                            this.setState({localUpdate:true,tips:tips,balance:parseFloat((balance - amount)),id:null,celebrate:true},() => {
    ...
    chooseOption = (id,e) => {
        ...
        if(this.state.item.owner_id == this.props.pageData.loggedInUserDetails.user_id){
            return;
        }
        this.setState({id:id,localUpdate:true});
        swal({
    ...
    render() {
        ...
        if(this.state.gateways){
            gatewaysHTML = <Gateways {...this.props} success={() => {
                ...
                setTimeout(() => {
                    Router.push(`/watch?id=${this.state.custom_url}`, `/watch/${this.state.custom_url}`)
                  },1000);
            }} successBank={() => {
                    ...
                    <div className="donationInnr">
                        <div>
                            <div className="totalDontn">
                                ...
                                <div className="text">
                                    {this.props.t("Wallet Balance: {{price}}",{price:Currency({...this.props,...perprice}).replace("<!-- -->","")})}
                                </div>
                            </div>
                            ...
                            this.state.item.tips.map((item,i) => {
                                ...
                                return (
                                    <div className="tipBtn" key={item.tip_id}>
                                        ...
                                        <span>{Currency({...this.props,...perprice}).replace("<!-- -->","")}</span>
                                        </button>
