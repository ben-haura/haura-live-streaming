...
class FixedMenu extends Component {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        }else if(!nextProps.pageData.loggedInUserDetails){
            return {
                notifications:[],
                loading: false
            }
    ...
    componentDidUpdate(prevProps,prevState){
        if(this.props.pageData.loggedInUserDetails && JSON.stringify(this.props.pageData.loggedInUserDetails) != JSON.stringify(prevProps.pageData.loggedInUserDetails)){
        ...
        axios.post(url, formData)
            .then(response => {
                if (response.data.error) {
                    ...
                    try{
                        SendMessageToApps({props:this.props,type:"loggedinUser",theme:this.props.pageData.themeMode,currentUrl:null,notifications:response.data.unread});
        ...
        this.props.socket.on('notifications', data => {
            if (this.props.pageData && this.props.pageData.loggedInUserDetails) {
                if (data.owner_id == this.props.pageData.loggedInUserDetails.user_id) {
                    SendMessageToApps({props:this.props,type:"loggedinUser",theme:this.props.pageData.themeMode,currentUrl:null,notifications:this.state.unread + 1});
    ...
    render() {
        ...
        return (
            <header id="header" className={`${!this.props.pageData.removeFixedMenu ? "fixed-top" : "nofixed-top"}`}>
