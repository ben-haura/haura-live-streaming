...
const CreateButtons = dynamic(() => import("../Header/CreateButtons"), {
    ssr: false,
});
const Notifications = dynamic(() => import("../Header/Notifications"), {
    ssr: false
});
const SettingMenus = dynamic(() => import("../Header/SettingMenus"), {
    ssr: false
});
...
const NotificationItems = dynamic(() => import("../Header/NotificationItems"), {
    ssr: false
});
const SettingsItems = dynamic(() => import("../Header/SettingsMenuItems"), {
    ssr: false
});
const CreateButtonsItems = dynamic(() => import("../Header/CreateButtonsItem"), {
    ssr: false
});
...
class Menu extends Component {
    constructor(props) {
        super(props)
        this.state = {
            search: props.pageData.searchTitleText ? props.pageData.searchTitleText : "",
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        }else if (nextProps.pageData.searchTitleText && nextProps.pageData.searchTitleText != prevState.search) {
            return { search: nextProps.pageData.searchTitleText ? nextProps.pageData.searchTitleText : "", style: false }
        } else{
            return null
        }
    }
    ...
    componentDidMount() {
        ...
        this.props.socket.on('notifications', data => {
            if (this.props.pageData && this.props.pageData.loggedInUserDetails) {
                if (data.owner_id == this.props.pageData.loggedInUserDetails.user_id) {
        ...
        this.props.socket.on("deleteNotifications", data => {
            ...
            if (this.props.pageData && this.props.pageData.loggedInUserDetails) {
                if (owner_id == this.props.pageData.loggedInUserDetails.user_id) {
    ...
    render() {
        if (!this.props.pageData || !this.props.pageData.menus || !this.props.pageData.menus.menus) {
            return null
        }
        let menus = null

        !this.props.mobileMenu ?
            menus = this.props.pageData.menus.menus.map(elem => {
                return (
                                            ...
                                            {  
                                                    this.props.pageData.loggedInUserDetails ?
                                                        <React.Fragment>
                                                            ...
                                                            { 
                                                                this.props.pageData.levelPermissions && parseInt(this.props.pageData.levelPermissions["member.allow_messages"]) == 1 ? 
                                                                    <li className="nav-item dropdown main">
                                                                        <Link href="/messanger" as="/messages" >
                                                                            <a className="nav-link bg-cnt">
                                                                                <span className="material-icons-outlined parent" data-icon="chat"></span>
                                                                            </a>
                                                                        </Link>
                                                                    </li>
                                                                : null
                        ...
                        <div className="MobeaderContent">
                            <div className="MobLeftSide">
                                <div className="MobmenuWrap">
                                    <nav className="navbar navbar-toggleable-lg">
                                        ...
                                        <div className={`collapse navbar-collapse bg-inverse MobMenuScroll${this.state.menuStyle}`}>
                                            ...
                                            {
                                                this.props.pageData.appSettings["fixed_header"] == 0 ? 
                                ...
                                {/* {
                                    !this.props.pageData.loggedInUserDetails ? */}
                                        <React.Fragment>
                                            <div className="MobSearchicon MobileSearchBtn" data-bs-toggle="collapse" data-bs-target="#MobSearchbox" aria-expanded="false" aria-controls="MobSearchbox">
                                                <span className="material-icons" data-icon="search"></span>
                                            </div>
                                ...
                                <div className="MobRightSide headerRightMenu">
                                    <ul className="mobRightNav ">
                                        <li className="dropdown main">
                                            <Link href="/" >
                                                <a className="nav-link bg-cnt">
                                                    <span className="material-icons-outlined" data-icon="home"></span>
                                                    <span className="title">{this.props.t("Home")}</span>
                                                </a>
                                            </Link>
                                        </li>
                                        ...
                                        <CreateButtons  {...this.props} type="mobile" style={this.state.type == "createbuttons" ? this.state.style : "none"} openToggle={this.openToggle} setCreateButtonsWrapperRef={this.setCreateButtonsWrapperRef} />
                                        {
                                            ...
                                            <li className="dropdown main">
                                                <Link href="/messanger" as="/messages" >
                                                    <a className="nav-link bg-cnt">
                                                        <span className="material-icons-outlined" data-icon="chat"></span>
                                                        <span className="title">{this.props.t("Messages")}</span>
                                                    </a>
                                                </Link>
                                            </li>
                                            : null
