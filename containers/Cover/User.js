...
class Cover extends Component {
    ...
    deleteUser = (e) => {
        ...
        let username = ""
        let usernameData= ""
        if(this.props.pageData.loggedInUserDetails && this.state.item.user_id != this.props.pageData.loggedInUserDetails.user_id){
    ...
    imageSize = (url) => {
        ...
        img.src = this.props.pageData.imageSuffix+url;
        return promise;
    }
    repositionCover = async (e) => {
        e.preventDefault()
        
        this.setState({localUpdate:true,reposition:true},async () => {
            uiMinJS('.coverphotoUsercnt img').draggable({disabled:false});
            const imageDimensions = await this.imageSize(this.state.item.cover);
    ...
    repositionSave = (e) => {
        ...
        url = `/members/reposition-cover?containerHeight=${$(".header-info-wrap").height()}`;
        axios.post(url, formData, config)
            .then(response => {
    ...
    openChat = () => {
        ...
        this.props.socket.emit('chatOpen', {user_id:this.props.pageData.loggedInUserDetails.user_id,resource_id:this.state.item.user_id})
    }
    render() {
        ...
        if(!this.state.reposition && this.state.item.cover_crop){
            ...
            if (splitVal[0] == "http:" || splitVal[0] == "https:") {
                ...
                coverImage = this.props.pageData.imageSuffix + image
            }
        }
        if (coverImage) {
            ...
            if (splitVal[0] == "http:" || splitVal[0] == "https:") {
                ...
                coverImage = this.props.pageData.imageSuffix + coverImage
            }
        }
        if (mainPhoto) { 
        ...
        let gif = false;
        if(coverImage && (coverImage.indexOf(".gif") > -1 || coverImage.indexOf(".GIF") > -1)){
            gif = true;
        }
        ...
        return (
            <React.Fragment>
                <div className={`header-info-wrap ${this.props.pageData.userProfilePage ? " subscription-urser" : " container"}`}>
                                ...
                                <React.Fragment>
                                    {/* <div className="box-overlay"></div> */}
                                    {
                                        ...
                                        <React.Fragment>
                                            ...
                                            {
                                                this.state.repositionDisplay == "block" && this.state.item.usercover && !gif ? 
                                ...
                                <div className="userInfo-block-content">
                                    <div className="userinfoLeft">
                                        ...
                                        <div className="user-profile-title">
                                            <h4>{this.state.item.displayname + " "}
                                                {
                                                    this.props.pageData.appSettings['member_verification'] == 1 && this.state.item.verified ?
                                                        <span className="verifiedUser" title={Translate(this.props, "verified")}><span className="material-icons" data-icon="check"></span>
                                                        </span>
                                                        : null
                                                }
                                                {
                                                    this.props.pageData.appSettings['member_featured'] == 1 && this.state.item.is_featured == 1 ?
                                                ...
                                                {
                                                    this.props.pageData.appSettings['member_sponsored'] == 1 && this.state.item.is_sponsored == 1 ?
                                                ...
                                                { 
                                                    this.props.pageData.appSettings['member_hot'] == 1 && this.state.item.is_hot == 1 ?
                                    ...
                                    <div className="user_followed">
                                       <div className="follow-cnt">
                                           {
                                               this.props.pageData.levelPermissions && parseInt(this.props.pageData.levelPermissions["member.allow_messages"]) == 1 && (!this.props.pageData.loggedInUserDetails || this.props.pageData.loggedInUserDetails.user_id != this.state.item.user_id) ? 
                                                    <a className="message follow" href="#" onClick={(e)=>{
                                                        e.preventDefault();
                                                        this.openChat();
                                                    }}>{this.props.t("Message")}</a>
                                            : null
                                            }
                                    ...
                                    <div className="LikeDislikeWrap">
                                        <ul className="LikeDislikeList jstyCenter">
                                            {
                                                this.props.pageData.appSettings[`${"member_like"}`] == 1 ?
                                                    <li>
                                            ...
                                            {
                                                this.props.pageData.appSettings[`${"member_dislike"}`] == 1 ?
                                                    <li>
                                            ...
                                            {
                                                this.props.pageData.appSettings[`${"member_favourite"}`] == 1 ?
                                                    <li>
                                            ...
                                            <SocialShare {...this.props} hideTitle={true} url={`/${this.state.item.username}`} title={this.state.item.displayname} imageSuffix={this.props.pageData.imageSuffix} media={this.state.item.avtar} />                                            
                                            {
                                        !this.props.profile ?
                                            <li>
                ...
                {
                        this.props.pageData.appSettings['below_cover'] ? 
                            <AdsIndex paddingTop="20px" className="below_cover" ads={this.props.pageData.appSettings['below_cover']} />
                        : null
                    }
            </React.Fragment>
        )
