    ...
    render() {
        ...
        if(this.state.gateways){
            gatewaysHTML = <Gateways {...this.props} success={() => {
                ...
                setTimeout(() => {
                    Router.push(`/channel?id=${this.state.item.custom_url}`, `/channel/${this.state.item.custom_url}`)
                  },1000);
            }} successBank={() => {
        ...
        let amount = this.props.t("Support: Pay {{price}} per month",{price:Currency({...this.props,...perprice})}).replace("<!-- -->","")
        return (
            <React.Fragment>
                {gatewaysHTML}
