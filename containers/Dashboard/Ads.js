...
class Ads extends React.Component {
    ...
    loadMoreContent(values) {
        ...
        let queryString = ""
        if (this.props.pageData.searchData) {
            queryString = Object.keys(this.props.pageData.searchData).map(key => key + '=' + this.props.pageData.searchData[key]).join('&');
            url = `${url}?${queryString}`
        }
    ...
    render() {
        ...
        if (this.props.pageData.categories) {

            categories.push({ key: 0, value: Translate(this.props, "Please Select Category") })
            this.props.pageData.categories.forEach(res => {
        ...
        let results = this.state.items.map(item => {
            ...
            return (
                <tr key={item.ad_id}>
                    ...
                    <td>{Currency({...this.props,...spent}).replace("<!-- -->","")}</td>
                    <td>
                    ...
                    <div className="col-12">
                        <div className="container">
                            <div className="row wallet">
                                <div className="col-md-2 wallet_amount">{Translate(this.props, "Wallet Total:")}{" "}<b>{Currency({...this.props,...wallet}).replace("<!-- -->","")}</b></div>
                                {
