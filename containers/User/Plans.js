...
class Plans extends React.Component{
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        if(prevState.localUpdate){
            ...
            return {
                ...
                gateways:null,
                memberSubscriptionPaymentStatus: nextProps.pageData.memberSubscriptionPaymentStatus,
    ...
    render () {
        ...
        if(this.state.viewPlan || this.state.viewPlan == 0){
            planHTML = <div className="popup_wrapper_cnt">
                            <div className="popup_cnt">
                                <div className="comments">
                                    <div className="popup_wrapper_cnt_header">
                                        <h2>{this.state.plans[this.state.viewPlan].title}</h2>
                                        <a onClick={(e) => {
                                            e.preventDefault();
                                            this.setState({localUpdate:true,viewPlan:null})
                                        }}  className="_close"><i></i></a>
                                    </div>
                                    <p className="plan-description">
                                        {this.state.plans[this.state.viewPlan].description}
                                    </p>
                                </div>
                            </div>
                        </div>
        ...
        if(this.state.gateways){
            gatewaysHTML = <Gateways {...this.props} success={() => {
                ...
                if(this.state.itemObj && this.state.itemObj.channel_id){
                    setTimeout(() => {
                        Router.push(`/channel?id=${this.state.itemObj.custom_url}`, `/channel/${this.state.itemObj.custom_url}`)
                    },1000);
                }else if(this.state.itemObj && this.state.itemObj.blog_id){
                    setTimeout(() => {
                        Router.push(`/blog?id=${this.state.itemObj.custom_url}`, `/blog/${this.state.itemObj.custom_url}`)
                    },1000);
                }else if(this.state.itemObj && this.state.itemObj.playlist_id){
                    setTimeout(() => {
                        Router.push(`/playlist?id=${this.state.itemObj.custom_url}`, `/playlist/${this.state.itemObj.custom_url}`)
                    },1000);
                }else if(this.state.itemObj && this.state.itemObj.audio_id){
                    setTimeout(() => {
                        Router.push(`/audio?id=${this.state.itemObj.custom_url}`, `/audio/${this.state.itemObj.custom_url}`)
                    },1000);
                }else if(this.state.itemObj && this.state.itemObj.video_id){
                    setTimeout(() => {
                        Router.push(`/watch?id=${this.state.itemObj.custom_url}`, `/watch/${this.state.itemObj.custom_url}`)
                    },1000);
                }else{
                    setTimeout(() => {
                        Router.push(`/member?id=${this.state.member.username}`, `/${this.state.member.username}`)
                    },1000);
                }
            }} successBank={() => {
        ...
        let plans = this.state.plans.map((plan,index) => {
            ...
            if (splitVal[0] == "http:" || splitVal[0] == "https:") {
                ...
                image = this.props.pageData.imageSuffix + image
            }
            ...
            return (
                <div className={`${this.state.userPrifile ? "gridColumn" : "gridColumn"}`} key={plan.member_plan_id}>
                    <div className="card mx-auto plancard">
                        <div className="card-body">
                            <div className="pname-img">
                                ...
                                <p className="m-0 pname">{plan.title}<br />{this.props.t("{{price}} / month",{price:Currency({...this.props,...perprice})}).replace("<!-- -->","")}
                                </p> 
                            </div>
                            ...
                            `${description}`
                                ...
                                <a href="#" onClick={(e) => {
                                    e.preventDefault();
                                    this.setState({viewPlan:index,localUpdate:true})
                                }}>{this.props.t("view more")}</a>
                               : null
                            }</h6>
                            <div className="row plan-options">
                                {
                                    this.props.pageData.loggedInUserDetails && this.state.user_id == this.props.pageData.loggedInUserDetails.user_id ?
                                    ...
                                    <div className="col-md-12 mt-3">
                                        {
                                            this.state.userSubscription && this.state.userSubscriptionID == plan.member_plan_id ? 
                                                this.props.pageData.appSettings['member_cancel_user_subscription'] == 1 ? 
            ...
            <React.Fragment>
                ...
                }
                {
                    !this.state.userPrifile ? 
                        <div className="gridContainer gridPlan">
                            {
                                plans
                            }
                        </div>
