...
class Video extends Component {
    constructor(props) {
        ...
        if (props.pageData.appSettings.uploadVideo == "1") {
            enableVideo = true
        }
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        if(prevState.localUpdate){
            ...
            let enableVideo = false
            if (nextProps.pageData.appSettings.uploadVideo == "1") {
                enableVideo = true
            }
            return {
                processing:false,
                ...
                openAddTip:false,
                tips:nextProps.pageData.editItem && nextProps.pageData.editItem.tips ? [...nextProps.pageData.editItem.tips] : [{amount:0}],
                previousTips:nextProps.pageData.editItem && nextProps.pageData.editItem.tips ? [...nextProps.pageData.editItem.tips] : [{amount:0}],
                chooseType: enableVideo ? "upload" : "import",
                editItem: nextProps.pageData.editItem,
                videoTitle: nextProps.pageData.editItem ? nextProps.pageData.editItem.title : null,
                videoDescription: nextProps.pageData.editItem ? nextProps.pageData.editItem.description : null,
                videoTags: nextProps.pageData.editItem && nextProps.pageData.editItem.tags ? nextProps.pageData.editItem.tags.split(',') : null,
                videoImage: null,
                ...
                error: null,
                sell_videos:nextProps.pageData.sell_videos ? true : false,
                plans:nextProps.pageData.plans ? nextProps.pageData.plans : [],
                videoAmountType: !nextProps.pageData.editItem ? "videotip" : (nextProps.pageData.editItem.mediaserver_stream_id || nextProps.pageData.editItem.channel_name ? "livestreaming" : "videotip")
            }
        }
    }
    componentDidMount(){
    ...
    onSubmit = model => {
        ...
        if(this.state.sell_videos && this.props.pageData.appSettings['video_commission_type']  == 1 && this.props.pageData.appSettings['video_commission_value'] > 0){
            if(model['price'] && parseFloat(model['price']) > 0){
                if(model['price'] <=  this.props.pageData.appSettings['video_commission_value']){
                    let perprice = {}
                    perprice['package'] = { price: this.props.pageData.appSettings['video_commission_value'] }
            ...
            .then(response => {
                if (response.data.error) {
                    ...
                    if(this.props.chooseVideos){
                        ...
                        Router.push(`/watch?id=${response.data.custom_url}`, `/watch/${response.data.custom_url}`)
                    }
                }
            }).catch(err => {
    ...
    uploadMedia = (e) => {
       ...
       var allowedExtensions = ['mp4','mov','webm','mpeg','3gp','avi','flv','ogg','mkv','mk3d','mks','wmv'];
        if (allowedExtensions.indexOf(extension) === -1) 
        {
            ...
            return false;
        }else if( parseInt(this.props.pageData.appSettings['video_upload_limit']) > 0 && e.size > parseInt(this.props.pageData.appSettings['video_upload_limit'])*1000000){
            alert(this.props.t('Maximum upload limit is {{upload_limit}}',{upload_limit:this.formatBytes(parseInt(this.props.pageData.appSettings['video_upload_limit'])*1000000)}));
            return false;
        }
            ...
            .then(response => {
                if (response.data.error) {
                    ...
                    if (key == "import-url") {
                        ...
                        this.setState({localUpdate:true, videoWidth: response.data.videoWidth, validating: false, id: response.data.id, success: true, videoTitle: response.data.name, videoImage: response.data.images[0] });
                    }
                }
            }).catch(err => {
    ...
    saveTips = (e) => {
        ...
        let tips = [...this.state.tips]
        let perprice = {}
        perprice['package'] = { price: this.props.pageData.appSettings[this.state.videoAmountType+'_commission_value'] }
        tips.forEach((item,index) => {
            if(parseFloat(item.amount) > 0){
                if(parseFloat(this.props.pageData.appSettings[this.state.videoAmountType+'_commission_value']) > 0 && parseInt(this.props.pageData.appSettings[this.state.videoAmountType+'_commission_type']) == 1 && parseFloat(item.amount) <= parseFloat(this.props.pageData.appSettings[this.state.videoAmountType+'_commission_value'])){
                    valid = false
                ...
                item.error = this.props.t("Enter amount must be greater than {{price}}.",{price : parseFloat(this.props.pageData.appSettings[this.state.videoAmountType+'_commission_value']) > 0 && parseInt(this.props.pageData.appSettings[this.state.videoAmountType+'_commission_type']) == 1 ? Currency({...this.props,...perprice}) : 0});
                valid = false;
            }
        })
    ...
    render() {
        ...
        if(this.state.chooseType == "upload" && this.state.sell_videos && (!this.state.editItem || this.state.editItem.type == 3)){
            ...
            if(this.props.pageData.appSettings['video_commission_type']  == 1 && this.props.pageData.appSettings['video_commission_value'] > 0){
                let perprice = {}
                perprice['package'] = { price: this.props.pageData.appSettings['video_commission_value'] }
                formFields.push({
                    key: "price_desc_1",
        ...
        if (this.props.pageData.videoArtists) {
            let artists = []

            this.props.pageData.videoArtists.forEach(res => {
                artists.push({ value: res.artist_id.toString(), key: res.title, label: res.title, image: this.props.pageData.imageSuffix + res.image })
            })
        ...
        if(this.props.pageData.levelPermissions["livestreaming.scheduled"] == 1 && this.props.pageData.livestreamingtype == 0 && this.state.editItem && this.state.editItem.scheduled){
            ...
            let currentTime = dateS.tz(this.props.pageData.defaultTimezone).toDate();
            formFields.push({
                key: "scheduled",
                        ...
                        <div className="VideoDetails-commentWrap tip_cnt">
                            ...
                            <div className="user_wallet">
                                <div className="row">
                                    <form>
                                        {
                                        parseFloat(this.props.pageData.appSettings[this.state.videoAmountType+'_commission_value']) > 0 && parseInt(this.props.pageData.appSettings[this.state.videoAmountType+'_commission_type']) == 1 ?
                ...
                }
                {
                    this.state.success ?
                            !this.state.channel_id ?
                               <React.Fragment>
                                    <Breadcrum {...this.props}  image={this.props.pageData['pageInfo']['banner'] ? this.props.pageData['pageInfo']['banner'] : this.props.pageData['subFolder']+"static/images/breadcumb-bg.jpg"} title={`${this.state.editItem ? "Edit" : "Create"} Video`} />
                        ...
                        <div className="videoBgWrap container" ref={this.myRef}>
                            {
                                this.state.enableUploadVideo || this.props.pageData.levelPermissions['video.embedcode'] == 1 ?
                                    <div className="user-area">
                                        <div className="container">
                                             <div className="BtnUpld">
                                                ...
                                                {
                                                    this.props.pageData.levelPermissions['video.embedcode'] == 1 ? 
