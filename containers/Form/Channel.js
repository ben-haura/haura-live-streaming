...
class Channel extends Component {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        if(prevState.localUpdate){
            ...
            return {
                title:`${nextProps.pageData.editItem ? "Edit" : "Create"} Channel` ,
    ...
    onSubmit = async model => {
        ...
        if(this.props.pageData.appSettings['channel_support_commission_type']  == 1 && this.props.pageData.appSettings['channel_support_commission_value'] > 0){
            if(model['channel_subscription_amount'] && parseFloat(model['channel_subscription_amount']) > 0){
                if(model['channel_subscription_amount'] <=  this.props.pageData.appSettings['channel_support_commission_value']){
                    let perprice = {}
                    perprice['package'] = { price: this.props.pageData.appSettings['channel_support_commission_value'] }
        ...
        if(this.state.editItem){
            url = "/channels/create";
            formData.append("id",this.state.editItem.channel_id)
        ...
        .then(response => {
            if(response.data.error){
                ...
                Router.push(`/channel?id=${response.data.custom_url}`,`/channel/${response.data.custom_url}`)
            }
        }).catch(err => {
    ...
    render(){        
        ...
        //support price
        if(this.props.pageData.appSettings['channel_support'] == 1){
            validator.push(
                {
        ...
        formFields.push(
            ...
            { key: "image", label: "Upload Image", type:"file",value:this.state.editItem && this.state.editItem.image ? this.props.pageData.imageSuffix+this.state.editItem.image : null }
        )

        if(this.props.pageData.channelCategories){
            let categories = []
            categories.push({key:0,value:0,label:"Please Select Category"})
            this.props.pageData.channelCategories.forEach(res => {
        ...
        if(this.props.pageData.channelArtists){
            let artists = []

            this.props.pageData.channelArtists.forEach(res => {
        ...
        if(this.props.pageData.appSettings.channel_adult == "1"){
            formFields.push({
                key: "adult",
                label: "",
                subtype:"single",
        ...
        return (
            <React.Fragment>
                { this.state.openPopup ?  <AddVideos {...this.props} chooseVideos={this.chooseVideos} closePopup={this.closePopup} title={Translate(this.props,"Add video to channel")} /> :null }
                    <Breadcrum {...this.props}  image={this.props.pageData['pageInfo']['banner'] ? this.props.pageData['pageInfo']['banner'] : this.props.pageData['subFolder']+"static/images/breadcumb-bg.jpg"} title={`${this.state.editItem ? "Edit" : "Create"} Channel`} />          
