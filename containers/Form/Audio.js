...
class Audio extends Component {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        if(prevState.localUpdate){
            ...
            return { 
                ...
                title: nextProps.pageData.editItem ? "Edit Audio" : "Create Audio",
        ...
        axios.post(url, formData, config)
            .then(response => {
                if (response.data.error) {
                    ...
                    this.setState({localUpdate:true, validating: false, id: response.data.id, success: true, audioTitle: response.data.name,chooseType:"audio",audio_url:response.data.audio_url },() => {
                        this.createPeakForm()
                    });
    ...
    onSubmit = model => {
        ...
        if(this.state.id){
            formData.append("audio_id",this.state.id)
        }
        if(this.state.peaks){
            ...
            .then(response => {
                if (response.data.error) {
                    ...
                    if(!this.state.editItem && this.state.peaks){
                        ...
                        .then(response1 => {
                            Router.push(`/audio?id=${response.data.custom_url}`, `/audio/${response.data.custom_url}`)
                        })
                    }else{
                        Router.push(`/audio?id=${response.data.custom_url}`, `/audio/${response.data.custom_url}`)
                    }
            ...
            <React.Fragment>
                {
                    this.state.chooseType ? 
                        <React.Fragment>
                            <Breadcrum {...this.props}  image={this.props.pageData['pageInfo']['banner'] ? this.props.pageData['pageInfo']['banner'] : this.props.pageData['subFolder']+"static/images/breadcumb-bg.jpg"} title={`${this.state.editItem ? "Edit" : "Create"} Audio`} />          
