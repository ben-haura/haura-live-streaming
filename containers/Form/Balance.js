...
class Balance extends Component {
    ...
    componentDidMount(){
        ...
        $(document).on("click",'.open_withdraw',function(e) {
            e.preventDefault();
            let user = that.props.pageData.user ? `&user=${that.props.pageData.user}` : "";
            let userAs = that.props.pageData.user ? `?user=${that.props.pageData.user}` : "";
            Router.push(
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        if(prevState.localUpdate){
           ...
           return {
                ...
                submitting: false,
                firstLoaded: true,
                user:nextProps.pageData.user ? true : false,
    ...
    onSubmit = model => {
        ...
        let user = that.props.pageData.user ? `&user=${that.props.pageData.user}` : "";
        let userAs = that.props.pageData.user ? `?user=${that.props.pageData.user}` : "";
        let price = this.props.pageData.levelPermissions['member.monetization_threshold_amount']
    ...
    render() {
        ...
        let wallet = {}
        wallet['package'] = { price: this.state.member ? this.state.member.wallet : this.props.pageData.loggedInUserDetails.wallet }

        let userBalance = {}
         userBalance['package'] = { price: parseFloat(this.state.member.balance ? this.state.member.balance : 0) }  
        let priceData = Currency({...this.props,...perclick}).replace("<!-- -->","")
        let formFields = [
            { key: "wallet", label: "Wallet Total",props: { readOnly: true }, value: Currency({...this.props,...wallet}).replace("<!-- -->","") },
            { key: "balance", label: "Available Balance",props: { readOnly: true }, value: Currency({...this.props,...userBalance}).replace("<!-- -->","") },
            { key: "paypal_email", label: "Paypal Email", value: this.state.member.paypal_email ? this.state.member.paypal_email : "",isRequired:true },
