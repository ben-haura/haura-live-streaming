...
class General extends Component {
    ...
    onSubmit = model => {
        ...
        if(model["phone_number"] && model['phone_number'] != this.state.member.phone_number && this.props.pageData.appSettings['twillio_enable'] == 1){
    ...
    render() {
        ...
        if (this.props.pageData.levelPermissions['member.username'] == 1) {
            validator.push({
                key: "username",
                validations: [
                    {
        ...
        if(this.props.pageData.appSettings['twillio_enable'] == 1){
            if(this.props.pageData.appSettings["signup_phone_number_required"] == 1){
                validator.push({
                    key: "phone_number",
                    validations: [
                        {
            ...
            formFields.push({ key: "phone_number",type: "phone_number", label: "Phone Number", value: this.state.member.phone_number,isRequired: this.props.pageData.appSettings["signup_phone_number_required"] == 1 ? true : false })
        }
        formFields.push(
            {
                key: "timezone", label: "Timezone", type: "select", options: timezone,
                value: this.state.member.timezone ? this.state.member.timezone : this.props.pageData.appSettings["member_default_timezone"]
            }
        )
        
        

        if(this.props.pageData.appSettings['video_donation'] == 1 && this.props.pageData.levelPermissions['video.donation'] == 1){
            formFields.push({ key: "paypal_email", label: "Donation PayPal Email", value: this.state.member.paypal_email })
        }
        if (this.props.pageData.levels && this.props.pageData.loggedInUserDetails.level_id == 1 && this.state.member.level_id != 1) {
            let levels = []

            this.props.pageData.levels.forEach(level => {
                levels.push({
        ...
        if (this.props.pageData.loggedInUserDetails.level_id == 1 && this.state.member.verificationFunctionality) {
            formFields.push({
