  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if(prevState.localUpdate){
        ...
        return {
          ...
          initialState: nextProps.pageData.editItem ? nextProps.pageData.editItem.description : "",
          title: nextProps.pageData.editItem ? "Edit Blog" : "Create Blog",
          category_id: nextProps.pageData.editItem
            ? nextProps.pageData.editItem.category_id
            : null,
          subcategory_id: nextProps.pageData.editItem
            ? nextProps.pageData.editItem.subcategory_id
            : null,
          subsubcategory_id: nextProps.pageData.editItem
            ? nextProps.pageData.editItem.subsubcategory_id
            : null,
          privacy: nextProps.pageData.editItem
            ? nextProps.pageData.editItem.view_privacy
            : "everyone",
          success: false,
          error: null,
...
componentDidUpdate(prevProps,prevState){
  ...
  onSubmit = async model => {
    ...
    if (this.state.editItem) {
      formData.append("id", this.state.editItem.blog_id);
    }
    ...
    axios
      ...
      .then(response => {
        if (response.data.error) {
          ...
          Router.push(
            `/blog?id=${response.data.custom_url}`,
  ...
  render() {
    ...
    let theme = ""
		if(this.props.pageData && this.props.pageData.themeMode){
			theme = this.props.pageData.themeMode
    }
    ...
    let htmlEditor = null;
    {
      this.state.editor
        ? (htmlEditor = (
            <Editor
              initialValue={this.state.initialState}
              apiKey={this.props.pageData.tinymceKey}
              init={{
                height: 700,
                menubar: true,
    ...
    let formFields = [
      ...
      },
      {
        ...
        label: "Upload Image",
        type: "file",
        value: this.state.editItem && this.state.editItem.image ?  this.props.pageData.imageSuffix +this.state.editItem.image : ""
      }
    ];

    if (this.props.pageData.blogCategories) {
      let categories = [];
      categories.push({ key: 0, value: 0, label: "Please Select Category" });
      this.props.pageData.blogCategories.forEach(res => {
        categories.push({
          key: res.category_id,
          label: res.title,
    ...
    if (this.props.pageData.appSettings.blog_adult == "1") {
      formFields.push({
        key: "adult",
        label: "",
        type: "checkbox",
        subtype:"single",
        value:[this.props.pageData.editItem && this.props.pageData.editItem.adult ? "1" : "0"],
        options: [
          {
            value: "1",
    ...
    formFields.push({
      key: "search",
      label: "",
      value:[this.props.pageData.editItem ? (this.props.pageData.editItem.search ? "1" : "0")  : "1"],
      type: "checkbox",
      subtype:"single",
      options: [
        {
    ...
    return (
      <React.Fragment>
          <Breadcrum {...this.props}  image={this.props.pageData['pageInfo']['banner'] ? this.props.pageData['pageInfo']['banner'] : this.props.pageData['subFolder']+"static/images/breadcumb-bg.jpg"}title={`${this.state.editItem ? "Edit" : "Create"} Blog`} />          
