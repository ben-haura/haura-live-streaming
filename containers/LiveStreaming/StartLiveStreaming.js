...
class Index extends React.Component{
    ...
    onUnload = () => {
        this.props.socket.emit('leaveRoom', {room:this.state.role == "host" ? "ptv_"+this.state.randNumber : this.state.video.channel_name,custom_url:this.state.custom_url})
    ...
    createAuthToken = () => {
        ...
        let url = '/live-streaming/access_token?channelName='+(this.state.role == "host" ? "ptv_"+this.state.randNumber : this.state.video.channel_name)+'&role='+(this.state.role == "host" ? "publisher" : "subcriber");
    ...
    createVideoStreaming = async () => {
        ...
        client.init({role:this.state.role,appID:this.props.pageData.agora_app_id,codec:"vp8"}).then((_) => {
            let data = {}
            data.role = this.state.role
            data.token = this.state.token
            data.channel = this.state.role == "host" ? "ptv_"+this.state.randNumber : this.state.video.channel_name
    ...
    startRecording(){
        if(this.state.role == "host"){
            let formData = new FormData();
            formData.append("channel","ptv_"+this.state.randNumber)
                        ...
                        <React.Fragment>
                            {
                                ...
                                <React.Fragment>
                                    ...
                                    <div className="videoWrapCnt" id="video" style={{ width: "100%", "height": "100%", position: "relative" }} >
                                        ...
                                        {/* {
                                        !this.props.needSubscription && this.props.width <= 992 && this.props.videoElem && this.props.videoElem.approve == 1 && this.props.videoElem.enable_chat == 1 && ( (this.props.videoElem.is_livestreaming == 1 && (this.props.videoElem.channel_name || this.props.videoElem.mediaserver_stream_id)) || this.props.videoElem.scheduled   ) ? 
                ...
                </div>
                {
                    this.state.allow_chat == 1 ? 
                        <div className="ls_sidbar">
                            <Chat {...this.props} finish={this.state.streamleave} deleteAll={true} channel={this.state.role == "host" ? "ptv_"+this.state.randNumber : this.state.video.channel_name} custom_url={this.state.custom_url} />
                        </div>    
                    : null
                }
            </div>
