...
module.exports = {
    ...
    insertScheduledFollowers: function (req, data) {
        return new Promise(function (resolve,reject) {
            req.getConnection(function (_, connection) {
                const date = dateTime.create().format("Y-m-d H:M:S")
                connection.query("INSERT INTO notifications (`owner_id`,`subject_type`,`subject_id`,`object_type`,`object_id`,`type`,`notification_send`,`creation_date`) SELECT owner_id,?,?,?,?,?,0,? FROM `scheduled_videos` WHERE video_id = ? ", [data.subject_type, data.subject_id, data.object_type, data.object_id, data.type, date, data.object_id], function (err, results) {
                    if (err)
                        resolve(false)
                    if (results) {
                        resolve(true);
