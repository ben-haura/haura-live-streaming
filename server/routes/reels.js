...
router.use(process.env.subFolder+':lng?/create-reel/:id?',(req,res,next) => {
...
router.use(process.env.subFolder+':lng?/reel/:id?',(req,res,next) => {
    middlewareEnable.isEnable(req,res,next,"reels",'view')
},controller.view);

router.use(process.env.subFolder+':lng?/story/:id?',(req,res,next) => {
