...
router.use(process.env.subFolder+'update-user-push-token',multer().none(), async (req, res) => {
    ...
    await globalModel.custom(req,"SELECT * FROM devices WHERE push_token = ?",[token]).then(async result => {
        ...
        if(oldToken && (!devices || devices.length == 0 || devices[0].push_token != oldToken)){
            await globalModel.delete(req,"devices","device_id",oldToken);
        }
        if(devices.length > 0){
            if(devices[0].push_token != oldToken && devices[0].owner_id != user_id)
                await globalModel.update(req,{owner_id: user_id,push_token:token,modified_date:formatted},"devices","device_id",token);
        }else{
