...
var validateFile = function (file, cb,req) {
  ...
  if(file.fieldname == "videoStories"){
    allowedFileTypes = /mp4|mov|webm|mpeg|3gp|avi|flv|ogg|mkv|mk3d|mks|wmv/
  }else if(file.fieldname == "audioStories"){
    allowedFileTypes = /"mp3|m4a|aac|wav|mp4|wma/
  }else if(req.allowedFileTypes){
...
var readS3Image = async (req,image,newimage) => {
  return new Promise(async function (resolve,reject) {
      ...
      var s3 = new aws.S3(wisabiStorage);
      s3.getObject({ Bucket: req.appSettings.s3_bucket, Key: trimImage }, 
        function(err, data) {
          if (!err) {
              const fs = require("fs")
...
module.exports = { upload, bytesToSize, uploadtoS3,readS3Image,makeDirectory };