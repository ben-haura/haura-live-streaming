...
exports.index = async (req, res) => {
    if(req.query.ids){
        ...
        for(let i=0;i<ids.length; i++){
            if(parseInt(ids[i]) > 0){
                await videoModel.delete(ids[i], req).then(() => {})
            }
        }
...
exports.approve = async (req, res) => {
    ...
    await globalModel.custom(req, "SELECT * from videos where video_id = ?", id).then(async result => {
        if (result && result.length) {
            let item = result[0]
            await globalModel.update(req, { approve: !item.approve }, "videos", "video_id", id).then(async () => {
                if (item.owner_id != req.user.user_id && !item.approve) {
                    let type = "videos_admin_approved"
                    if(item.type == 11){
                        type = "live_admin_approved"
                        socketio.getIO().emit('liveStreamApproved', {
                            "videoID": id
                        });
                    }
                    notifications.insert(req, {notChangeOwnerID:true, owner_id: item.owner_id, type: type, subject_type: "users", subject_id: item.owner_id, object_type: "videos", object_id: item.video_id, insert: true }).then(() => {
 
                    }).catch(() => {

                    })
                } else if (item.owner_id != req.user.user_id && item.approve) {
                    let type = "videos_admin_disapproved"
                    if(item.type == 11){
                        type = "live_admin_disapproved"
                    }
                    notifications.insert(req, {notChangeOwnerID:true, owner_id: item.owner_id, type:type, subject_type: "users", subject_id: item.owner_id, object_type: "videos", object_id: item.video_id, insert: true }).then(() => {
                ...
                if(!item.approve){
                    if(item["type"] != 3 && !item["scheduled"]){
                        await notificationModel.insertFollowNotifications(req,{subject_type:"users",subject_id:item.owner_id,object_type:"videos",object_id:item.video_id,type:"members_followed"}).then(() => {
                    ...
                    let userObject = {}
                    await globalModel.custom(req, "SELECT * from users where user_id = ?", item.owner_id).then(async result => {
                        if (result && result.length) {
                            userObject = result[0]
                        }
                    })

                    if(parseInt(item.type) == 11 || parseInt(item.type) == 10){
                        if(!item['scheduled']){
                            let dataNotification = {}
                            dataNotification["type"] = "live_video"
                            dataNotification["owner_id"] = item.owner_id
                            dataNotification["object_type"] = "videos"
                            dataNotification["object_id"] =  item.video_id
                            dataNotification.notChangeOwnerID = true
                            notificationModel.sendPoints(req,dataNotification,userObject.level_id);
                        }else{
                            await notificationModel.insertFollowNotifications(req,{subject_type:"users",subject_id:item.owner_id,object_type:"videos",object_id:item.video_id,type:"scheduled_live"}).then(() => {
                        ...
                        dataNotification["object_id"] =  item.video_id
                        dataNotification.notChangeOwnerID = true
                        notificationModel.sendPoints(req,dataNotification,req.user.level_id,userObject.level_id);
                    }
                }
...
exports.featured = async (req, res) => {
    ...
    await globalModel.custom(req, "SELECT * from videos where video_id = ?", id).then(async result => {
        if (result && result.length) {
            let item = result[0]
            await globalModel.update(req, { is_featured: !item.is_featured }, "videos", "video_id", id).then(() => {
                if (item.owner_id != req.user.user_id && !item.is_featured) {
                    notifications.insert(req, { owner_id: item.owner_id, type: "videos_featured", subject_type: "users", subject_id: item.owner_id, object_type: "videos", object_id: item.video_id, insert: true }).then(() => {
...
exports.sponsored = async (req, res) => {
    ...
    await globalModel.custom(req, "SELECT * from videos where video_id = ?", id).then(async result => {
        if (result && result.length) {
            let item = result[0]
            await globalModel.update(req, { is_sponsored: !item.is_sponsored }, "videos", "video_id", id).then(() => {
                if (item.owner_id != req.user.user_id && !item.is_sponsored) {
                    notifications.insert(req, { owner_id: item.owner_id, type: "videos_sponsored", subject_type: "users", subject_id: item.owner_id, object_type: "videos", object_id: item.video_id, insert: true }).then(() => {
...
exports.hot = async (req, res) => {
    ...
    await globalModel.custom(req, "SELECT * from videos where video_id = ?", id).then(async result => {
        if (result && result.length) {
            let item = result[0]
            await globalModel.update(req, { is_hot: !item.is_hot }, "videos", "video_id", id).then(() => {
                if (item.owner_id != req.user.user_id && !item.is_hot) {
                    notifications.insert(req, { owner_id: item.owner_id, type: "videos_hot", subject_type: "users", subject_id: item.owner_id, object_type: "videos", object_id: item.video_id, insert: true }).then(() => {
...
exports.delete = async (req, res) => {
    ...
    await videoModel.delete(id, req).then(() => {
        res.redirect(backURL)
        return
    })
}
    ...
    reg_form.handle(req, {
        success: function (form) {
            ...
            if (!artist_id) {
                ...
                globalModel.create(req, form.data, 'artists').then(() => {
                })
            } else
...
exports.deleteArtist = async (req, res) => {
    ...
    globalModel.delete(req, "artists", "artist_id", id).then(() => {
...
exports.createGallery = async(req,res) => {
    ...
    reg_form.handle(req, {
        success: function (form) {
            ...
            if (!photo_id) {
                globalModel.create(req, form.data, 'artist_photos').then(() => {})
            } else
...
exports.deleteGallery = async(req,res) => {
    ...
    await globalModel.custom(req,"DELETE FROM artist_photos WHERE photo_id = ?",[id]).then(() => {
        res.redirect(backURL)
        return
    })
}
        ...
        for(let i=0;i<ids.length; i++){
            if(parseInt(ids[i]) > 0){
                ...
                if(existingArtist){
                    ...
                    await globalModel.delete(req, "artists", "artist_id", id).then(() => {})
                }
            }
        }
        res.redirect(req.originalUrl.split("?")[0]);
...
exports.addCategories = async (req, res) => {
    ...
    if (!category_id) {
        ...
        let editCat = '<a class="btn btn-primary btn-xs" href="' + process.env.ADMIN_SLUG + '/videos/categories/add/' + id + '">Edit</a>';
...
exports.levels = async (req, res) => {
    ...
    reg_form.handle(req, {
        success: function (form) {
            permission.insertUpdate(req, res, form.data, level_id, "video").then(() => {
...
exports.settings = async (req, res) => {
    ...
    var reg_form = forms.create({
        ...
        // video_advanced_grid: fields.string({
        //     label: "Show videos advanced grid in browse pages",
        //     choices: { "1": "Yes", "0": "No" },
        //     widget: widgets.select({ "classes": ["select"] }),
        //     fieldsetClasses: "form_fieldset",
        //     cssClasses: { "field": ["form-group"] },
        //     value:  settings.getSetting(req, "video_advanced_grid", '0')
        // }),
        // video_carousel_home: fields.string({
        //     choices: { "1": "Yes", "0": "No" },
        //     widget: widgets.select({ "classes": ["select"] }),
        //     label: "Show videos on home page in carousel?",
        //     fieldsetClasses: "form_fieldset",
        //     cssClasses: { "field": ["form-group"] },
        //     value: settings.getSetting(req, "video_carousel_home", '0').toString()
        // }),
        // video_adv_slider: fields.string({
        //     choices: { "1": "Yes", "0": "No" },
        //     widget: widgets.select({ "classes": ["select"] }),
        //     label: "Show advanced slider on Landing Page?",
        //     fieldsetClasses: "form_fieldset",
        //     cssClasses: { "field": ["form-group"] },
        //     value: settings.getSetting(req, "video_adv_slider", '0').toString()
        // }),
        ...
        player_type: fields.string({
            choices: { "videojs": "Video JS Player", "element": "Media Element JS Player" },
            widget: widgets.select({ "classes": ["select"] }),
            label: "Video Player",
            fieldsetClasses: "form_fieldset",
            cssClasses: { "field": ["form-group"] },
            value: settings.getSetting(req, "player_type", 'element').toString()
        }),
        video_process_type: fields.string({
            label: "Max Processes Allowed",
            cssClasses: { "field": ["form-group"] },
            widget: widgets.text({ "classes": ["form-control"] }),
            value: settings.getSetting(req, "video_process_type", '1')
        }),
        video_process_type_label: fields.string({
        ...
        twitch_api_secret: fields.string({
            label: "Twitch Client Id",
            cssClasses: { "field": ["form-group"] },
            widget: widgets.text({ "classes": ["form-control"] }),
            value: req.loguserallowed ? "****" : settings.getSetting(req, "twitch_api_secret", '')
        }),
    ...
    reg_form.handle(req, {
        success: function (form) {
            ...
            delete form.data["video_process_type_label"]
...
exports.youtubeImport = async (req, res) => {
    ...
    if (query.search) {
        let LimitNum = parseInt(query.LimitNum) ? parseInt(query.LimitNum) : 20;
        ...
        youTube.search(query.search, LimitNum, params, function (error, itemResults) {
            if(error){
                const url = req.originalUrl.replace(process.env.ADMIN_SLUG, '');
                return res.render('admin/videos/youtube', {type:1, adminLink: process.env.ADMIN_SLUG, results: [], categories: categories, query: {}, paggingData: "", nav: url, title: "Import Youtube Videos" });
            }
            ...
            youTube.getById(ids.join(','), function (error, results) {
                ...
                if (itemResults.prevPageToken) {
                    let previousUrl = process.env.PUBLIC_URL + process.env.ADMIN_SLUG + "/videos/import/youtube"
                ...
                if (itemResults.nextPageToken) {
                    let nextUrl = process.env.PUBLIC_URL + process.env.ADMIN_SLUG + "/videos/import/youtube"
        ...
        let reqData = {}
        reqData["order"] = "date"
        reqData["id"] = channelID
        reqData["maxResults"] = parseInt(query.LimitNum) ? parseInt(query.LimitNum) : 20;
        if(query.page)
            reqData["pageToken"] = query.page
        ...
        .then(async function (response) {
            if(response.data){
                ...
                let itemResults = {}
                itemResults.nextPageToken = response.data.nextPageToken
                itemResults.prevPageToken = response.data.prevPageToken
                ...
                youTube.getById(ids.join(','), function (error, results) {
                    ...
                    if (itemResults.prevPageToken) {
                        ...
                        if (query.channel) {
                            queryString.push("channel=" + query.channel)
                        }
                        if (query.limit) {
                            queryString.push("limit=" + query.limit)
                        }
                    ...
                    if (itemResults.nextPageToken) {
                        ...
                        let queryString = []
                        if (query.type) {
                            queryString.push("type=" + query.type)
                        }
                        if (query.channel) {
                            queryString.push("channel=" + query.channel)
                        }
...
exports.createImportedVideos = async (req, res) => {
    ...
    if (user_email) {
        await globalModel.custom(req, "SELECT * FROM users LEFT JOIN userdetails on userdetails.user_id = users.user_id where email = ?", [user_email]).then(result => {
    ...
    if(req.body.channelID){
        //channel import videos
        let data = req.body
        data.fromAdmin = 1;
        data.owner_id = user.user_id
        data.level_id = user.level_id
        let insertData = {}
        insertData.owner_id = user.user_id
        insertData.importchannel_id = req.body.channelID;
        insertData.owner_email = user.email
        insertData.owner_language = user.language
        insertData.owner_displayname = user.displayname
        insertData.params = JSON.stringify(data)
        insertData["creation_date"] = formatted
        insertData["completed"] = 0
        await globalModel.create(req,insertData,'channelvideoimports').then(() => {
        ...
        globalModel.custom(req, "SELECT * FROM videos WHERE type = 1 AND code = ?", [video.video_id]).then(resultData => {
            if (resultData) {
                ...
                if (resultsDatas.length == 0) {
                    ...
                    globalModel.create(req, videoObject, "videos").then(() => {
...
exports.dailyMotionImport = async (req, res) => {
    let params = {}
    const query = { ...req.query }
    if (query.page) {
        params = { pageToken: query.page }
