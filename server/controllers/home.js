...
exports.index = async (req, res, next) => {
    ...
    if (!cachedData) {
        await videoModel.getVideos(req, { is_not_hot: 1, is_not_sponsored: 1, is_not_featured: 1, limit: req.query.themeType == 2 ? 10 : 10,print:"1" }).then(result => {
            if (result && result.length > 0) {
...
exports.homeData = async (req,res) => {
    ...
    if (req.appSettings["enable_channel"] == 1) {
        ...
        if (req.appSettings["channel_featured"] == 1) {
            ...
            if (!cachedData) {
                await channelModel.getChannels(req, { is_featured: 1, limit: req.query.themeType == 2 ? 10 : 5, orderby: "random" }).then(result => {
                    if (result && result.length > 0) {
        ...
        if (req.appSettings["channel_sponsored"] == 1) {
            ...
            if (!cachedData) {
                await channelModel.getChannels(req, { is_sponsored: 1, limit: req.query.themeType == 2 ? 10 : 5, orderby: "random" }).then(result => {
                    if (result && result.length > 0) {
        ...
        await channelModel.findAllCommunity(0,req,res,req.query["themeType"] == 2 ? 10 : 4,0,0).then(result => {
            if (result && result.length > 0) {
        ...
        if (req.appSettings["channel_hot"] == 1) {
            ...
            if (!cachedData) {
                await channelModel.getChannels(req, { is_hot: 1, limit: req.query.themeType == 2 ? 10 : 5, orderby: "random" }).then(result => {
                    if (result && result.length > 0) {
        ...
        if (!cachedRecentChannel) {
            await channelModel.getChannels(req, { is_not_hot: 1, is_not_sponsored: 1, is_not_featured: 1, limit: req.query.themeType == 2 ? 10 : 5 }).then(result => {
                if (result && result.length > 0) {
...
exports.getImportVideosYoutube = async (req, results,channelInfo) => {
    return new Promise(async function (resolve, reject) {
        ...
        async.forEachOf(results, async function (videoData, i, callback) {
            await commonFunction.getVideoData("youtube", videoData.id.id, req, null).then(async result => {
                if (result) {
...
exports.cronFunction = async(req,res,next) => {
    return new Promise(function(resolve, reject) { 
        req.getConnection(function(err,connection){
            ...
            //update video and movies task if not run from last 6 hours
            connection.query("UPDATE tasks SET started = 0,start_time = ? WHERE (type = ? || type = ? || type = ? || type = ? || type = ? || type = ? || type = ? || type = ? || type = ? || type = ? || type = ? || type = ?) AND started = 1 AND DATE_ADD(start_time, INTERVAL 4 hour) <= ? ", [dateTime.create().format("Y-m-d H:M:S"),"channelVideoImport","notifications","movieImportIMDB","videoEncode","movieVideoEncode","movieImportIMDB","movieImportChangeIMDB","autoDeleteVideos","autoDeleteImportedVideos","newsletters","reelsVideoEncode","storiesVideoEncode",dateTime.create().format("Y-m-d H:M:S")], async function (err, results, fields) {
