...
exports.castView = async(req,res) => {
    ...
    res.query = req.query
    req.query = {}
    req.app.render(req, res,  '/cast-and-crew', req.query);
...
exports.browseSeason = async (req,res) => {
    ...
    res.query = req.query
    req.query = {}
    req.app.render(req, res, '/season', req.query);
}
exports.create = async (req, res) => {
    ...
    if(req.query.selectType == "series")
        req.app.render(req, res, '/create-series', req.query);
    else
...
exports.successul = async (req, res, next) => {
    ...
    if(id != req.session.id && (gateway != 2)){
        await commonFunction.getGeneralInfo(req, res, "page_not_found")
        res.query = req.query
        req.query = {}
        req.app.render(req, res, '/page-not-found', req.query);
    ...
    if(gateway == "2" && stripeToken){
        ...
        //create order
        await globalModel.create(req, {owner_id:req.user.user_id,gateway_id:2,state:"initial",creation_date:currentDate,source_type:movieSeriesType,source_id:id}, "orders").then(result => {
            if (result) {
                req.session.movie_user_id = req.user.user_id
                req.session.orderId = result.insertId
            } else {
    ...
    if(commissionType == 2 && commissionTypeValue > 0){
        commission_amount = (price * (commissionTypeValue/100)).toFixed(2);
    ...
    if(isValidResult) {
        await globalModel.create(req, {type:movieSeriesType,id:movie.movie_id, owner_id: req.session.movie_user_id, package_id: 0, status: gatewayResponse.state.toLowerCase(),creation_date: currentDate, modified_date: currentDate, gateway_profile_id: gatewayResponse.transaction_id,order_id:req.session.orderId }, "subscriptions").then(async result => {
            ...
            await moviePurchaseModel.insertTransaction(req, {gateway_id: (gateway ? gateway : 1) , order_id:req.session.orderId,admin_commission:commission_amount, gateway_transaction_id: gatewayResponse.transaction_id, owner_id: movie.owner_id ,sender_id:req.session.movie_user_id, state: gatewayResponse.state.toLowerCase(), price: parseFloat(price).toFixed(2) - commission_amount, currency: req.appSettings.payment_default_currency, creation_date: currentDate, modified_date: currentDate,id:movie.movie_id,type:movieSeriesType },true).then(async result => {
                //update user balance
...
exports.browse = async (req, res) => {
    ...
    if(req.contentType == "movies")
        req.app.render(req, res, '/movies', req.query);
    else
        req.app.render(req, res, '/series', req.query);
}
