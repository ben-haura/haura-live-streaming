const { validationResult } = require('express-validator'),
    ...
    reelModel = require("../../models/reels"),
...
exports.delete = async (req, res) => {
    ...
    await reelModel.delete(req.item.reel_id, req).then(result => {
        if (result) {
    ...
    if (Object.keys(reelObject).length && id) {
        ...
        if(!req.body.fromEdit){
            if(req.appSettings["video_ffmpeg_path"] && req.appSettings["video_ffmpeg_path"] != "" && reelObject.video_location.indexOf(".mp4") == -1){
                ...
                //upload video to s3
                if (req.appSettings.upload_system == "s3"  || req.appSettings.upload_system == "wisabi") {
                    await s3Upload(req, req.serverDirectoryPath +"/public"+ insertObject['video_location'], insertObject['video_location']).then(result => {
                        //remove local file
                        commonFunction.deleteImage(req, res, insertObject['video_location'], 'locale')
                    }).catch(err => {
