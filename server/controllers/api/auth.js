  ...
  return getModel.then(async user => {
    ...
    globalModel.custom(req, "SELECT vars,type FROM emailtemplates WHERE type = ?", ["lost_password"]).then(async resultsType => {
      if (resultsType) {
        ...
        return res.json({ status: true,code:phone ? code : "" });
      }else{
      ...
      .then(doMatch => {
        if (doMatch || phone_number) {
          ...
          if(req.session.fromAPP){
            ...
            devicedModel.createDevice(req, {owner_id:req.session.user,device_udid:req.session.device_udid,push_token:req.session.push_token,creation_date:formatted,modified_date:formatted}).then(async result => {
  ...
  bcrypt
    ...
    .then(async hashedPassword => {
      ...
      return globalModel.create(req, fieldValues, 'users').then(async result => {
        ...
        //follow user
        let sql = "SELECT tools_user_subscribe.user_id FROM tools_user_subscribe INNER JOIN users on users.user_id = tools_user_subscribe.user_id WHERE users.approve = 1 AND users.active = 1"
        await globalModel.custom(req, sql, []).then(results => {
            ...
            if(result && result.length > 0){
              result.forEach(async user => {
                ...
                await followModel.insert(data,req,res);
              })
            }
        })

        //channel subscribe
        let sqlChannel = "SELECT tools_channel_subscribe.channel_id FROM tools_channel_subscribe INNER JOIN channels on channels.channel_id = tools_channel_subscribe.channel_id "
        await globalModel.custom(req, sqlChannel, []).then(results => {
          let result = JSON.parse(JSON.stringify(results))
          if(result && result.length > 0){
            result.forEach(async user => {
              let data = {}
              data['type'] = "channels"
              data['id'] = user.channel_id
              data['owner_id'] = user_id
              data['creation_date'] = formatted
              await followModel.insert(data,req,res);
            })
