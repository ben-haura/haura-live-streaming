...
exports.delete = async(req,res) => {
    ...
    if(type == "channels"){
        ...
        column = "audio_id"
    }else if(type == "cast_crew_members"){
        ...
        await globalModel.create(req,data,"comments").then(async result => {
            if(result){
                ...
                if(parent_id){ 
                    ...
                    //insert notification
                    notifications.insert(req, {insert:true,owner_id: commentObj[(data.type == "members" ? "user_id" : "owner_id")], type: type+"_reply_comment", subject_type: "users", subject_id: req.user.user_id, object_type: "comments", object_id: comment_id }).then(result => {

                    }).catch(err => {

                    })
                    await commentModel.getReplies({limit:1,offset:0,data_comment_id:comment_id,comment_id:parent_id},req,res).then(result => {
                        if(result){
                   ...
                   // }
                    ...
                    await commentModel.getComments({limit:1,offset:0,data_comment_id:comment_id},req,res).then(result => {
                        if(result){
