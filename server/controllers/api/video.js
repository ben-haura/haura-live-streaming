...
exports.autoDeleteVideos = async(req,data) => {
    return new Promise(async function (resolve) {
        if(data.active != 2)
            await globalModel.update(req, {active:2}, "tools_delete_videos", 'delete_video_id', data.delete_video_id);
        ...
        let condition = []
        let sql = "SELECT * from videos LEFT JOIN users ON users.user_id = videos.owner_id where 1=1 "
        if(category_id != 0){
        ...
        await globalModel.custom(req,sql,condition).then(async results => {
            if(results && results.length > 0){
                ...
                for(let i = 0;i<items.length; i++){
                    ...
                    await globalModel.update(req, {last_process_video_id:items[i].video_id,delete_video_count:deleteVideo}, "tools_delete_videos", 'delete_video_id', data.delete_video_id);
                ...
                if(items.length <100){
                    await globalModel.update(req, {active:1}, "tools_delete_videos", 'delete_video_id', data.delete_video_id); 
                }
            }else{
                await globalModel.update(req, {active:1}, "tools_delete_videos", 'delete_video_id', data.delete_video_id); 
            }
    ...
    return new Promise(async function (resolve) {
        ...
        await globalModel.custom(req,sql,condition).then(async results => {
            if(results && results.length > 0){
                ...
                let deleteVideo = data.delete_video_count
                for(let i = 0;i<items.length; i++){
                    await globalModel.update(req, {last_process_video_id:items[i].video_id}, "tools_remove_videos", 'remove_video_id', data.remove_video_id);
                    //fetch video info
                    let url = ""
                    if(items[i].type == 1){
                    ...
                    await commonFunction.getUploadVideoInfo(url).then(async result => {
                        if (!result.status) {
                            ...
                            await commonFunction.getVideoData(result.provider, result.id, req, result).then(async result => {
                                if (result) {
                                    let responseData = result
                                    if(responseData.image && (items[i].image.indexOf("http://") > -1 || items[i].image.indexOf("https://") > -1)){
                                        await globalModel.update(req, {image:responseData.image}, "videos", 'video_id', items[i].video_id);
                                    }
                                } else{
                                    deleteVideo = parseInt(deleteVideo)+1
                                    await globalModel.update(req, {delete_video_count:deleteVideo}, "tools_remove_videos", 'remove_video_id', data.remove_video_id);
                                    await videoModel.delete(items[i].video_id, req).then(result => {
                                        if (result) {
                                            commonFunction.deleteImage(req, "", "", "video", items[i])
                                            socketio.getIO().emit('videoDeleted', {
                ...
                if(items.length <100){
                    await globalModel.update(req, {active:1}, "tools_remove_videos", 'remove_video_id', data.remove_video_id); 
                }
            }else{
                await globalModel.update(req, {active:1}, "tools_remove_videos", 'remove_video_id', data.remove_video_id); 
            }
        ...
        globalModel.custom(req,"SELECT * from videos where mediaserver_stream_id = ? AND is_livestreaming = 0 ORDER BY video_id DESC LIMIT 1",[id]).then(async result => {
            if(result){
                ...
                if(results && results.length){
                    ...
                    if(!videoObj.image && parseInt(req.appSettings["antserver_media_hlssupported"]) == 1){
                        insertObject['image'] = `/${req.appSettings['antserver_media_app']}/previews/`+req.body.vodName+".png"
                        if(n > -1){
...
exports.createKey = async(req,res) => {
    ...
    let reqData = {listenerHookURL:process.env.PUBLIC_URL+"/api/live-streaming/status","streamId":req.body.streamingId,"type": "liveStream","name": req.user.user_id+" - "+req.user.displayname+" live streaming",rtmpURL:"rtmp://"+(req.appSettings['antserver_media_url'].replace("https://","").replace("http://",""))+`/${req.appSettings['antserver_media_app']}`}
    var config = {
        method: 'post',
    ...
    .then(function (response) {
       if(response.data.status == "created"){
        ...
        var config = {
            method: 'get',
            url: req.appSettings["antserver_media_url"].replace("https://","http://")+`:5080/${req.appSettings['antserver_media_app']}/rest/v2/broadcasts/`+req.body.streamingId+"/token?expireDate="+(myDate.getTime() / 1000).toFixed(0)+"&type=publish",
            headers: { 
                'Content-Type': 'application/json;charset=utf-8'
            },
            //httpsAgent: agent
...
exports.removeViewer = async(req,res) => {
    res.send({})
    let customUrl = req.body.custom_url
    globalModel.custom(req,"UPDATE videos SET total_viewer = IF(total_viewer = 0,1, total_viewer) - 1 WHERE custom_url = ?",[customUrl]).then(result => {
...
exports.goLive = async(req,res) => {
    ...
    await globalModel.custom(req, "SELECT owner_id FROM videos WHERE video_id = ?", id).then(async result => {
        if (result && result.length) {
    ...
    globalModel.custom(req, "UPDATE videos SET scheduled = null,is_livestreaming = 1 WHERE video_id = '"+id+"'").then(result => {
    ...
    notificationModel.insertFollowNotifications(req,{subject_type:"users",subject_id:req.user.user_id,object_type:"videos",object_id:id,type:"livestreaming_live"}).then(result => {

    }).catch(err => {

    })

    notifications.insertScheduledFollowers(req, {  subject_type: "users", subject_id: videoObject.owner_id, object_type: "videos", object_id: id, type: "videos_reminder" }).then(result => {
    ...
    notificationModel.sendPoints(req,dataScheduledNotification,req.user.level_id);
...
exports.finishStreaming = async (req,res) => {
    ...
    await globalModel.update(req,{logo_active:0,overlay_active:0},"live_brands","user_id",req.user.user_id).then(result=>{
        
    })
    await globalModel.update(req,{show:0},"live_banners","user_id",req.user.user_id).then(result=>{
    ...
    var config = {
        method: 'post',
        url: req.appSettings["antserver_media_url"].replace("https://","http://")+`:5080/${req.appSettings['antserver_media_app']}/rest/v2/broadcasts/`+streamID+"/stop",
        headers: { 
            'Content-Type': 'application/json;charset=utf-8'
        },
        //httpsAgent: agent
...
exports.deleteStream = (req,streamID) => {
    ...
    var config = {
        method: 'delete',
        url: req.appSettings["antserver_media_url"].replace("https://","http://")+`:5080/${req.appSettings['antserver_media_app']}/rest/v2/broadcasts/`+streamID,
        headers: { 
            'Content-Type': 'application/json;charset=utf-8'
        },
        //httpsAgent: agent
...
exports.mediaStreamRecord = async(req,res) => {
    ...
    var config = {
        method: 'put',
        url: req.appSettings["antserver_media_url"].replace("https://","http://")+`:5080/${req.appSettings['antserver_media_app']}/rest/v2/broadcasts/`+streamID+"/recording/true",
        headers: { 
            'Content-Type': 'application/json;charset=utf-8'
        },
        //httpsAgent: agent
...
exports.reminder = async(req,res) => {
    ...
    await videoModel.isScheduleActive(req,req.user.user_id,data.video_id).then(result => {
        if(result){
...
exports.browse = async (req, res) => {
    ...
    if(req.body.videoPurchased){
        ...
        data.purchase_user_id = req.body.purchase_user_id ? req.body.purchase_user_id : req.body.video_user_id
    }
...
exports.create = async (req, res) => {
    ...
    if (!Object.keys(videoObject).length || !videoObject.custom_url) {
        ...
        if(!req.liveStreaming){
            ...
            if(req.appSettings['live_streaming_type'] == 0){
                if(typeof req.levelPermissions["livestreaming.approve"] == "undefined" || req.levelPermissions["livestreaming.approve"] == "1"){
                    insertObject["approve"] = 1;
                }else
                    insertObject['approve'] = 0;
            }else{
    ...
    if(req.liveStreaming && !req.body.scheduled && insertObject["approve"] == 1){
        insertObject['is_livestreaming'] = 1;
    }
    ...
    if (id) {
        //update existing video
        await globalModel.update(req, insertObject, "videos", 'video_id', id).then(async result => {
            ...
            //if(insertObject["scheduled"]){                
                await videoModel.findByCustomUrl(videoObject['custom_url'] ? videoObject['custom_url'] : insertObject['custom_url'],req,res).then(result => {
                    if(result){
                        if(result.scheduled){
            ...
            //}
            res.send({editItem:videoObj,approved:videoObj ? videoObj.approve : "", id: id, message:videoObject['custom_url'] ?  constant.video.EDIT : constant.video.SUCCESS, custom_url: videoObject['custom_url'] ? videoObject['custom_url'] : insertObject['custom_url'] });
            if(req.body.fromEdit){
                id = null
                res.end()
                return
        ...
        await globalModel.create(req, insertObject, "videos").then(async result => {
            if (result) {
                ...
                if(req.liveStreaming && !insertObject["scheduled"] && parseInt(videoObj.approve) == 1){
                ...
                if(req.body.channel_id){
                    ...
                    if(insertObject["type"] != 3 && !insertObject["scheduled"] && parseInt(videoObj.approve) == 1){
                ...
                if(parseInt(videoObj.approve) == 1){
                    if(req.liveStreaming){
                        if(!insertObject['scheduled']){
                            ...
                            await notificationModel.insertFollowNotifications(req,{subject_type:"users",subject_id:req.user.user_id,object_type:"videos",object_id:result.insertId,type:"scheduled_live"}).then(result => {
                ...
                res.send({editItem:videoObj,approved:videoObj ? videoObj.approve : "", id: result.insertId, message: constant.video.SUCCESS, custom_url: insertObject['custom_url'],title:insertObject['title'],image:imagevideo });
            } else {
                return res.send({ error: fieldErrors.errors([{ msg: constant.general.DATABSE }], true), status: errorCodes.invalid }).end();
            }
        }).catch(err => {
        ...
        async.forEachOf(tips, async function (tipData, i, callback) {
            ...
            if( price > 0){
                ...
                if(!tip.tip_id || forceInsert){
                    ...
                    //update
                    await globalModel.update(req, insertObject, "tips", 'tip_id', tip.tip_id).then(result => {});
                }
            }else if(tip.tip_id){
                //delete entry
...
exports.convertVideo = async(req,videoObject) => {
    return new Promise(async (resolve,reject) =>  {
        ...
        await module.exports.executeFFMPEG(command, req.serverDirectoryPath + path_240, 240, orgPath, FFMPEGpath, watermarkImage, req).then(async result => {
            ...
            await globalModel.update(req, updatedObject, "videos", "video_id", id).then(result => {
            }).catch(error => {
            })
            is_validVideo = true
        }).catch(err => {
        ...
        if ((videoResolution >= 640 || videoResolution == 0) && is_validVideo && req.appSettings["video_upload_videos_type"].indexOf("360") > -1) {
            await module.exports.executeFFMPEG(command, req.serverDirectoryPath + path_640, 640, orgPath, FFMPEGpath, watermarkImage, req).then(async result => {
                ...
                await globalModel.update(req, updatedObject, "videos", "video_id", id).then(async result => {
                }).catch(error => {
        ...
        if (is_validVideo) {
            notifications.insert(req, {owner_id:videoObject.owner_id,insert:true, type: "videos_processed_complete", subject_type: "users", subject_id: videoObject.owner_id, object_type: "videos", object_id: id,forceInsert:true }).then(result => {

            }).catch(err => {

            })
            notificationModel.insertFollowNotifications(req, { subject_type: "users", subject_id: videoObject.owner_id, object_type: "videos", object_id: id, type: "members_followed" }).then(result => {

            }).catch(err => {

            })
        } else {
            notifications.insert(req, {owner_id:videoObject.owner_id,insert:true, type: "videos_processed_failed", subject_type: "users", subject_id: videoObject.owner_id, object_type: "videos", object_id: id,forceInsert:true }).then(result => {
...
exports.updatedBrands = async (req) => {
    let brand = {}
    await globalModel.custom(req,"SELECT * FROM live_brands WHERE user_id = ?",[req.user.user_id]).then(async result => {
        if(result && result.length > 0){
    ...
    await globalModel.custom(req,"SELECT * FROM live_banners WHERE user_id = ? AND `show` = 1",[req.user.user_id]).then(async result => {
        if(result && result.length > 0){
            let data = {}
            data.user_id = req.user.user_id
...
exports.showHideBanner = async(req,res) => {
    ...
    let banner = null
    await globalModel.custom(req,"SELECT * FROM live_banners WHERE banner_id = ?",[banner_id]).then(async result => {
        if(result && result.length > 0){
    ...
    if(parseInt(banner.show) == 0){
        if(parseInt(banner.ticker) == 1){
            //get previous ticker
            await globalModel.custom(req,"SELECT * FROM live_banners WHERE user_id = ? AND ticker = ? AND `show` = 1",[req.user.user_id,1]).then(async result => {
                if(result && result.length > 0){
                    previousTicker = JSON.parse(JSON.stringify(result))[0];
                }
                if(previousTicker.banner_id){
                    //hide previous ticker
                    await globalModel.update(req,{show:0},"live_banners","banner_id",previousTicker.banner_id).then(result => {})
                }
            })
        }else{
            //get previous banner
            await globalModel.custom(req,"SELECT * FROM live_banners WHERE user_id = ? AND ticker = ? AND `show` = 1",[req.user.user_id,0]).then(async result => {
                if(result && result.length > 0){
                    previousTicker = JSON.parse(JSON.stringify(result))[0];
                }
                if(previousTicker.banner_id){
    ...
    await globalModel.update(req,{show:banner.show == 1 ? 0 : 1},"live_banners","banner_id",banner_id).then(result=>{
        if(result){
...
exports.statusBrandsImages = async(req,res) => {
    ...
    await globalModel.update(req,insertObject,"live_brands","brand_id",brand.brand_id).then(result=>{
        if(result){
            insertObject["user_id"] = req.user.user_id
...
exports.deleteBanner = async(req,res) => {
    ...
    await globalModel.delete(req,"live_banners","banner_id",banner_id).then(result=>{
        if(result){
...
exports.createBrandLogoOverlay = async(req,res) => {
    let insertObject = {}
    let brand = null
    await globalModel.custom(req,"SELECT * FROM live_brands WHERE user_id = ?",[req.user.user_id,0]).then(async result => {
        if(result && result.length > 0){
            brand = JSON.parse(JSON.stringify(result))[0];
        }
    })
    ...
    if(brand){
        ...
        await globalModel.create(req,insertObject,"live_brands").then(result=>{
            if(result){
                insertObject["user_id"] = req.user.user_id
                socketio.getIO().emit('updateBrandLive', insertObject);
            }
...
exports.createBanner = async(req,res) => {
    ...
    if(banner_id){
        ...
        await globalModel.update(req,insertObject,"live_banners","banner_id",banner_id).then(result=>{
            if(result){
